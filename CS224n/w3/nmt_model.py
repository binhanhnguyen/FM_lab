...
class NMT(nn.Module):
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int], grader_params=None) -> Tuple[
        ...
        X_cnn_out = self.post_embed_cnn(X_permuted)
        X = X_cnn_out.permute(2, 0, 1)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        Y_timesteps = torch.split(Y, 1, dim=0)

        for Y_t in Y_timesteps:
            Y_t_squeezed = torch.squeeze(Y_t, 0)
            Ybar_t = torch.cat([Y_t_squeezed, o_prev], dim=1)
            dec_state, o_t, _ = self.step(Ybar_t, dec_state,
                                          enc_hiddens, enc_hiddens_proj,
                                          enc_masks)
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        e_t_unsq = torch.bmm(dec_hidden.unsqueeze(1), enc_hiddens_proj.transpose(1, 2))
        e_t = e_t_unsq.squeeze(1)
        ...
        a_t_unsq = torch.bmm(alpha_t.unsqueeze(1), enc_hiddens)
        a_t = a_t_unsq.squeeze(1)
