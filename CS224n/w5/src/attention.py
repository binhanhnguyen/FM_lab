...
def precompute_rotary_emb(dim, max_positions):
    ...
    freqs = torch.ger(t, inv_freq) # (max_positions, dim/2)
...
def apply_rotary_emb(x, rope_cache):
    ...
    x_reshaped = x.float().reshape(B, nh, T, hs // 2, 2)
    x_complex = torch.view_as_complex(x_reshaped)
    rope_complex = torch.view_as_complex(cos_sin)
    rope_complex = rope_complex.view(1, 1, T, hs // 2)

    x_rotated_complex = x_complex * rope_complex

    x_out = torch.view_as_real(x_rotated_complex)
    rotated_x = x_out.reshape(B, nh, T, hs).type_as(x)
    ### END YOUR CODE ###
    return rotated_x
