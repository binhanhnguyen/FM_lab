...
class CharCorruptionDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if length < min_len:
             ...
             trunc_len = random.randint(min_len, min(length, max_len))
             start_idx = random.randint(0, length - trunc_len)
             trunc_doc = document[start_idx : start_idx + trunc_len]

        trunc_len = len(trunc_doc)
        mask_len = max(1, int(trunc_len * 0.25)) 
        ...
        ids = [self.stoi[c] for c in masked_string]
